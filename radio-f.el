...
;; Refer to commit 38af9286 for details
...
(defun radio-f--view-gatekeeper (image-data image-type)
  "Inspect the object returned from `radio-f--fetch-artwork' for image
data. If present, the object is sent to `radio-f--stylize-artwork.'
for further processing. If the object does not contain image data,
locally provided artwork, if present, is used instead.

IMAGE-DATA and IMAGE-TYPE are generated by either the callback from
`radio-f--fetch-artwork' or the locally provided artwork."
  (let* ((station (radio-f--get-current-station-data))
         (visual (plist-get station :visual))
         (artwork
          (if (and image-data
              ...
              (cons image-data image-type)
            (when visual
              (radio-f--load-local-artwork visual))))
         (image
          (when artwork
